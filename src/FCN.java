...
public class FCN extends Layer {
    ...
    public FCN ( double[][][] i, int o, double l, int s, int st) {
        ...
        double[][] temp = new double[i[0].length][length * width];
    ...
    public double[][] forward() {
        ...
        if(!this.isLast()) {
            ...
            for(int i = 0; i < this.output.length; i++){
                for(int j = 0; j < this.output[0].length; j++){
                    this.preactivate[i][j] = this.output[i][j];
                }
            }
        ...
        System.out.println("index " + this.getIndex() + "output" + (Arrays.toString(this.output[95])));
        return this.output;
    }
    ...
    public double[][] flatten(double[][][] i, int s, int st){
        ...
        double[][] temp = new double[i[0].length][i.length * i[0][0].length];
        int count = 0;
    ...
    /**
     * @implNote used for last layer to standardize 'strength' of each guess into probabilities: [0, 1]
     */
    public void softmax(){
        ...
        if(this.isLast()){
            ...
            for(int i = 0; i < this.output.length; i++){
                ...
                double sum = 0;
                for(int j = 0; j < this.output[i].length; j++){
                    sum += Math.exp(this.output[i][j]);
                }

                for(int j = 0; j < this.output[i].length; j++){
                    //softmax = e ^ z_i / Σ e ^ z_j
                    double eZi = Math.exp(this.output[i][j]);
                    this.output[i][j] = eZi / sum;
    ...
    /**
     * @implNote Multiclass classifier for all series in batch (s:st)
    ...
    public double[] guess(){
        ...
        if(!this.isLast){
            ...
            //guess[i] = argmax(this.output[i])
            this.guess = new double[this.output.length];
            for(int i = 0; i < this.output.length; i++){
                this.guess[i] = this.argmax(this.output[i]);
//                BINARY CLASSIFICATION .GUESS()
...
//                }
    ...
    /**
     * Returns index of maximum value (label)
     * @param arr: array to get max of
     * @return argmax - index of max
     */
     ...
     * @implNote L(x) = Sparse Categorical Cross Entropy
     */
    public double loss(){
        System.out.println("ANSWER" + this.answer.length);
        double batchLoss = 0;
        double[] guess = this.guess();


        double correct = 0;
        double total = this.st - this.s;

        for(int i = this.s; i < this.st; i++){
            //index of true label = ans[i]
            int ans = this.answer[i];

            //window[sample][p(correct class)]
            batchLoss += Math.log(this.output[i][ans]);
        ...
        double valAcc;
        double valCorr = 0;
        for(int i = this.st; i < guess.length; i++){
            if(this.answer[i] == guess[i]){
                valCorr++;
            }
        }
        int valTot = guess.length - this.st;
        ...
        valAcc =  valCorr / valTot;
        System.out.println("VALACC" + valAcc);
        System.out.println("GUESS" + Arrays.toString(this.guess));
        System.out.println("ANS" + Arrays.toString(this.answer));
        System.out.println("S" + this.s);
        System.out.println("ST" + this.st);

        System.out.println();
        return -1.0 * batchLoss / (this.st - this.s);
    }
    ...
    public void backward(){
        ...
        if(this.isLast()){
            ...
            //internally calculates local gradient of softmax operation
            //multiplies by upstream (grad)
            grad = this.dSoftmax(grad);
    ...
    /**
     * @implNote guesses are actually probability values for each category output from the softmax, for last layer
    ...
    public double[][] dLoss(){
        ...
        double[][] grad = new double[this.st - this.s][1];

        for(int i = 0; i < grad.length; i++){
            int ans = answer[i];
            //all else would be 0 so only need 1 value stored
            grad[i][0] = -1 / this.output[i][ans];
            //            //yhat = guesses
            //            double yhat = this.output[i + this.s][0];
            //            //y = ground truth
            //            double y = this.answer[i + this.s];
    ...
    public double[][] dSigmoid(double[][] u){
        ...
        for(int i = 0; i < u.length; i++){
            //sigmoid formula
            double sgmd = 1 / (1 + Math.exp(-1 * this.preactivate[i + this.s][0]));
     ...
     * @param u: upstream gradient ∂L/∂u
     * @return 2D tensor of downstream gradients
     * @implNote e.g. if if u = σ(e), calculates ∂L/∂e = ∂L/∂u * ∂u/∂e
     * */
    public double[][] dSoftmax(double[][] u){
        int numCats = this.output[0].length;
        double[][] grad = new double[this.st - this.s][numCats];

        //for each sample
        for(int i = 0; i < u.length; i++){

            // index of where predicted probability is
            int ans = answer[i];
            //for each category's predicted probability
            for(int j = 0; j < numCats; j++){
                double S = this.output[i][j];

                if(j == ans){
                    grad[i][j] = u[i][0] * (S * (1 - S));
                }
                else{
                    grad[i][j] = u[i][0] * -1 * (S * this.output[i][ans]);
                }
    ...
    /**
     * @implNote upstream gradient is typically tensor after dSeLU or dSoftmax/dSigmoid
    ...
    public double[][][] deflatten(double[][] flatten){
        ...
        double[][][] t = new double[this.m][this.st - this.s][this.p];
        ...
        for(int j = this.s; j < this.st; j++){
            ...
            for(int i = 0; i < this.m; i++){
                ...
                for(int k = 0; k < this.p; k++ ){
                    ...
                    t[i][j - this.s][k] = flatten[j][count];
                    count++;
                }
            }
        }
