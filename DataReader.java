...
//
    ...
    /**
     * Overloaded method to choose which columns are used
     * @param c: array of which columns to make w series out of
     *
     * @param a: col index of labels corresponding to the data
     *
     * @param p: window size
     *
     * @return tensor of dim c.length x w x p
     */
    public double[][][] makeSeries(int[] c, int a, int p){
        int numSeries = this.data[a].length - (p + 1);
        // [numFeatures][numSeries][seriesLength]
        double[][][] series = new double[c.length][numSeries][p];

        //each feature (column) to make series out of
        for(int i = 0; i < c.length; i++){
            series[i] = this.makeSeries(c[i], p);
        }

        //solution vector:
        this.y = new int[numSeries];
        double[] ans = this.data[a];

        for(int j = 0; j < numSeries; j++){
            //i.e. [0, p-1]: series, [p]: class label
            this.y[j] = (int) ans[j + p];
        }
    ...
    /**
     * Overloaded method for multichannel input
     * @implNote must have [series[i].length ≅ w mod t]
     * @param series: set of series to make windows out of
     * @param w: number of training samples per window
     * @param t: number of validation samples per window
     * @return 4D tensor of folds: <br> [numFolds][numFeatures][w + t][seriesLength]
     */
    public double[][][][] makeFolds(double[][][] series, int w, int t){
        //total number of series (per feature)
        int total = series[0].length;

        //(total - w) = number of available series for training window construction
        //t = step size
        int numFolds = (total - w) / t;
        int numFeatures = series.length;
        int numSeries = w + t;
        int seriesLength = series[0][0].length;

        double[][][][] temp = new double[numFolds][numFeatures][numSeries][seriesLength];
        this.Y = new int[numFolds][numSeries];

        //for each fold
        for(int i = 0; i < numFolds; i++){
            //for each feature; e.g. {return, vol, etc...}
            for(int j = 0; j < numFeatures; j++){

                for(int k = 0; k < numSeries; k++){
                    // i * t = number of steps into series (t = step size, each fold is 1 step)
                    temp[i][j][k] = series[j][i * t + k];
                }
            }
            //solution(series[i]) = solution[i] (indices line up)
            for(int k1 = 0; k1 < numSeries; k1++){
                this.Y[i][k1] = this.y[i * t + k1];
            }
