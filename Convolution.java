...
public class Convolution extends Layer  {
    ...
    public void setInput(double[][][] input, int[] answer){
    ...
    /**
     * normalizes using all unique vals:
     *      each series beginning (n - (p + 1))[0] is unique
    ...
    public void normalize(double[][] arr){
        ...
        //calculate mean using only training vals
        for(int i = 0; i < this.st; i++){
            sum += arr[i][0];

        }

        double mean = sum / (this.st);

        //reset sum
        sum = 0;


        //calculate sd (only training vals)
        for(int k = 0; k < this.st; k++){

            //(xi - mean)^2
            double diff = arr[k][0] - mean;
            sum += Math.pow(diff, 2);

        }
        // last series
        for(int k = 1; k < arr[0].length; k++){
            double diff = arr[this.st - 1][k];
            sum += Math.pow(diff, 2);
        }
    ...
    public double[][][] forward() {
        ...
        System.out.println("index " + this.getIndex() + "output" + (Arrays.toString(this.output[0][st])));
    ...
    */
    ...
    public double[][][] convolve() {
        ...
        for(int g = 0; g < this.numFilters; g++){
            ...
            for(int h = this.s; h < this.input[0].length; h++){
    ...
    private void addBias() {
        ...
        for ( int h = 0; h < this.numFilters; h++ ) {
            for ( int i = this.s; i < this.input[0].length; i++ ) {
                for ( int j = 0; j < this.output[h][i].length; j++ ) {

                    //add bias by channel
                    this.output[h][i][j] += this.bias[h];
                }
            }
        }
    }
    ...
    */
        ...
        for(int i = 0; i < u.length; i++){
            for(int j = this.s; j < this.st; j++){
                for(int k = 0; k < u[0][0].length; k++){
                    ...
                    //if (x > 0): ∂L/∂u *  λ
                    if(preactivate[i][j - this.s][k] > 0){
                        u[i][j - this.s][k] *= lambda;
                    }
                    else{
                        u[i][j - this.s][k] *= (lambda * alpha * Math.exp(preactivate[i][j][k]) );
                    }
    ...
    public double[][][] dInput(double[][][] u){
        double[][][] grad = new double[this.input.length][this.st - this.s][this.input[0][0].length];
        double sum = 0;
                    ...
                    for (int l = 0; l < this.numFilters; l++) {
                        ...
                        for(int m = 0; m < this.input[0][0].length; m++) {
                            ...
                            if(m < k){
                                ...
                                sum += this.filters[l][i][this.kernel - 1] * u[l][j - this.s][m];
                            }
                            else{
                                ...
                                while(count >= 0 && it >= 0){
                                    if(it == k ){
                                        sum += this.filters[l][i][count] * u[l][j - this.s][m];
                                    }
    ...
    public double[][][] dResid(double[][][] u){
        ...
        double[][][] grad = new double[this.preone.length][this.st - this.s][this.preone[0][0].length];
        double sum = 0;
                ...
                for (int j = this.s; j < this.st; j++) {
                    ...
                    for (int k = 0; k < grad[0][0].length; k++) {
                        ...
                        for (int l = 0; l < this.output.length; l++) {
                            sum += u[l][j - this.s][k] * this.one[l][i];
                        }
            ...
            for(int j = 0; j < this.one[0].length; j++){
                ...
                for(int k = this.s; k < this.st; k++){
                    ...
                    for(int l = 0; l < this.input[0][0].length; l++){
                        sum += u[i][k - this.s][l] * this.preone[j][k][l];
                    }
                }
